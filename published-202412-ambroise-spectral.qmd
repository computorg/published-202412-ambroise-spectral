...
#| label: fig-balls-bridge
#| fig-cap: "Balls (left) versus Bridge (right). The inertia of each structure is the sum of the squared distances represented by grey lines."
grid.arrange(
...
Considering two centroïds, the normalized average of the difference between the balls inertia and that of the Bridge (see [Appendix](#gain)) constitutes the basis of our affinity measure between two regions:
$$
\begin{aligned}
\frac{I_{kl} - B_{kl}}{(n_k+n_l)\|\boldsymbol \mu_k - \boldsymbol \mu_l\|^2} &= \frac{\sum_{\boldsymbol{x_i} \in \mathcal V_k} \langle \boldsymbol{x_i} - \boldsymbol{\mu}_k \vert \boldsymbol{\mu}_l - \boldsymbol{\mu}_k \rangle_+^2 + \sum_{\boldsymbol{x_i} \in \mathcal V_l} \langle \boldsymbol{x_i} - \boldsymbol{\mu}_l \vert \boldsymbol{\mu}_k - \boldsymbol{\mu}_l\rangle_+^2}{(n_k+n_l)\|\boldsymbol \mu_k - \boldsymbol \mu_l\|^4},\\
...
::: {#fig-interpretation layout="[[1,1], [1,1]]"}

![Margin with close centroids](figures/svm-interpretation-3.svg)

![Density of the $\alpha_i$ for close centroids](figures/histo-alphai-3.svg)

![Margin with well separated centroids](figures/svm-interpretation-10.svg)

![Density of the $\alpha_i$ for well separated centroids](figures/histo-alphai-10.svg)
...
![Vector quantization](figures/spectral-bridges-1.svg)

![Affinity computation](figures/spectral-bridges-2.svg)

![Spectral clustering](figures/spectral-bridges-3-4.svg)
...
Determining the optimal value of $m$ using the eigengap is not straightforward. As the affinity matrix dimension increases, the number of eigenvalues grows, reducing gaps between them. This makes direct comparisons unreliable. To address this, we use the ratio $\rho = (\lambda_{K+1} - \lambda_K) / \lambda_K$. This metric is positive and measures the relative difference between consecutive eigenvalues. It facilitates meaningful comparisons across different values of $m$. A high value of $R$ suggests high clustering quality, whereas lower values indicate weaker performance.
...
| UCI Breast Cancer | $30$  | $569$   | $2$      | $37.3\%$, $62.7\%$                                                                        |
...
Table: Datasets Summary & Class Balance Class proportions are presented in ascending order starting from label $0$.
...
![ARI and NMI scores of Spectral Bridges with varying values of $m$.](figures/nodes_vs_score.svg){#fig-m-vs-score}
...
![Varying $n$, fixed $m = 10$](figures/size_vs_time.svg)

![Varying $m$, fixed $n = 5000$](figures/nodes_vs_time.svg)
...
![ARI and NMI scores of k-means++ (pink), EM (green), Ward Clustering (red), GIT (blue), and Spectral Bridges (purple).](figures/mnist_pca_summary.svg)
=======
![](figures/mnist_pca_summary.pdf)
...
|         | SB        | $\mathbf{0.5789}$ | $\mathbf{0.6875}$ | $\mathbf{0.7110}$ | $\mathbf{0.6983}$ | $\mathbf{0.6619}$ |
...
|         | SB        | $\mathbf{0.6592}$ | $\mathbf{0.7616}$ | $\mathbf{0.7895}$ | $\mathbf{0.7846}$ | $\mathbf{0.7628}$ |

Comparison of clustering algorithms on PCA embedded and full MNIST with ARI and NMI metrics. The highest mean scores among tested algorithms are in bold.
:::
...
![ARI and NMI scores of k-means++ (pink), EM (green), Ward Clustering (red), GIT (blue), Spectral Bridges (purple).](figures/mnist_umap_summary.svg)
...
|         | SB        | $\mathbf{0.9304}$ | $\mathbf{0.9318}$ | $\mathbf{0.9315}$ | $\mathbf{0.9332}$ |
| **NMI** | KM        | $0.8664$          | $0.8586$          | $0.8574$          | $0.8641$          |
|         | EM        | $0.8908$          | $0.9025$          | $0.8916$          | $0.8826$          |
|         | WC        | $0.9047$          | $0.8733$          | $0.8721$          | $0.8785$          |
|         | GIT       | $0.9081$          | $0.9172$          | $\mathbf{0.9183}$ | $0.9184$          |
|         | SB        | $\mathbf{0.9182}$ | $\mathbf{0.9198}$ | $0.9125$          | $\mathbf{0.9214}$ |

Comparison of clustering algorithms on UMAP embedded MNIST with ARI and NMI metrics. The highest mean scores among tested algorithms are in bold.
:::

Note the Spectral Bridges algorithm shines particularly even with quite simple dimension reduction algorithms.
...
![k-means++](figures/KMumap.jpg)

![Spectral Bridges](figures/SBumap.jpg)

![Ground Truth](figures/GTumap.jpg)
...
:::{#fig-cancer-scores layout-nrow=2}

![ARI and NMI scores of k-means++ (pink), EM (green), Ward Clustering (red), GIT (blue), and Spectral Bridges (purple) on the UCI Breast Cancer dataset.](figures/cancer_summary.svg)

| Metric  | Algorithm               | UCI Breast Cancer |
|---------|-------------------------|-------------------|
| **ARI** | KM                      | $0.6675$          |
|         | EM                      | $\mathbf{0.7718}$ |
|         | WC                      | $0.5750$          |
...
Comparison of clustering algorithms on the UCI Breast Cancer dataset with ARI and NMI metrics. The highest mean scores among tested algorithms are in bold.
:::
...
![Impossible](figures/impossible.jpg)

![Moons](figures/moons.jpg)

![Circles](figures/circles.jpg)

![Smile](figures/smile.jpg)
...
:::{#fig-synthetic-scores layout-nrow=2}

![ARI and NMI scores of Spectral Bridges and competitors on standard synthetic toy datasets.](figures/synthetic_summary.svg)

| Metric  | Algorithm | Impossible        | Moons             | Circles           | Smile             |
|---------|-----------|-------------------|-------------------|-------------------|-------------------|
| **ARI** | DB        | $0.8180$          | $0.9294$          | $0.9920$          | $\mathbf{1.0000}$ |
|         | KM        | $0.5984$          | $0.2390$          | $-0.0009$         | $0.5522$          |
|         | EM        | $0.6112$          | $0.4897$          | $-0.0009$         | $0.7526$          |
|         | WC        | $0.6368$          | $0.4298$          | $0.0068$          | $0.5548$          |
|         | GIT       | $0.8895$          | $\mathbf{0.9920}$ | $\mathbf{1.0000}$ | $0.5693$          |
|         | SB        | $\mathbf{0.9996}$ | $0.9912$          | $\mathbf{1.0000}$ | $\mathbf{1.0000}$ |
| **NMI** | DB        | $0.8823$          | $0.8825$          | $0.9803$          | $\mathbf{1.0000}$ |
|         | KM        | $0.6936$          | $0.1807$          | $0.0000$          | $0.6126$          |
|         | EM        | $0.7021$          | $0.3905$          | $0.0001$          | $0.8001$          |
|         | WC        | $0.7505$          | $0.4690$          | $0.0056$          | $0.6120$          |
|         | GIT       | $0.9103$          | $\mathbf{0.9812}$ | $\mathbf{1.0000}$ | $0.6210$          |
|         | SB        | $\mathbf{0.9995}$ | $0.9787$          | $\mathbf{1.0000}$ | $\mathbf{1.0000}$ |


Comparison of clustering algorithms on toy datasets with ARI and NMI metrics. The highest mean scores among tested algorithms are in bold.
:::
...
![Clean](figures/clean_impossible.jpg)

![Uniform noise](figures/uniform_noise_impossible.jpg)

![Gaussian noise](figures/gaussian_noise_impossible.jpg)
